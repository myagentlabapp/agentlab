...
# 域名从环境变量读（部署期注入），未设置时回退示例值
cd /mnt/storage/agent-tenant-platform/frontend
pkill -f '[v]ite --host' 2>/dev/null
sleep 2
VITE_ALLOWED_HOSTS="${VITE_ALLOWED_HOSTS:-example.com,localhost}" setsid nohup npm run dev -- --host 0.0.0.0 --port 3000 > /home/yy/vite.log 2>&1 < /dev/null &
sleep 5
